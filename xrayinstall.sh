...
setDokodemoDoorUnblockNetflixOutbounds() {
	...
	read -r -p "请输入http访问对应的端口:" httpPort
	read -r -p "请输入https访问对应的端口:" httpsPort
	if [[ -n "${setIP}" ]]; then
		cat <<EOF >${configPath}10_ipv4_outbounds.json
{
  "outbounds": [
...
# 只需要任意门协议，安装Xray-core，实现任意门，不需要多余工具
onlyInstallDokodemoDoor() {
	echoContent skyBlue "\n========================只安装Xray-core，实现任意门，不需要多余工具============================"
	echo '安装工具'
	echoContent skyBlue "\n进度  $1/${totalProgress} : 安装工具"
	# 修复ubuntu个别系统问题
	if [[ "${release}" == "ubuntu" ]]; then
